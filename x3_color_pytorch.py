...
def X3(y, iters, batch_sz, num_dict_features=None, D=None):
    ...
    D = torch.randn(3*patch_sz**2, k).float().cuda(0)
    ...
    for i in range(iters):
        ...
        x = x.unfold(1, patch_sz, 5).unfold(2, patch_sz, 8).unfold(3, 3, 1).contiguous()
        x = x.view(x.size(0)*x.size(1)*x.size(2)*x.size(3), x.size(4)*x.size(5), x.size(-1))
        x = whiten(x)
